...
class UIManager {
    ...
    setCurrentFilter(filter) {
        ...
        // Reset load more state when changing filters
        if (this.currentFilter !== filter) {
            this.loadMoreState.allTraits = [];
            this.loadMoreState.currentDisplayed = 0;
        }
    ...
    displayTokens(tokens, skipSelectionUpdate = false, hasLoadingWheels = false) {
        ...
        // Si es la primera carga o no es traits, limpiar todo
        const isFirstLoad = this.loadMoreState.currentDisplayed === 0 || this.currentFilter !== 'traits';
        ...
        if (isFirstLoad) {
            ...
            const loadMoreBtn = tokensGrid.querySelector('.load-more-button');
            if (loadMoreBtn) {
                loadMoreBtn.remove();
            }
            ...
            // Reset load more state for traits
            if (this.currentFilter === 'traits') {
                this.loadMoreState.allTraits = tokens;
                this.loadMoreState.currentDisplayed = 0;
            }
        }

        // 🚨 TEST N1: Limitar a máximo 150 traits en primera carga
        const maxTraits = 150;
        let traitsToDisplay = tokens;
        
        if (this.currentFilter === 'traits') {
            if (isFirstLoad && tokens.length > maxTraits) {
                console.log(`📊 Limitando display inicial a ${maxTraits} traits de ${tokens.length} totales`);
                traitsToDisplay = tokens.slice(0, maxTraits);
                this.loadMoreState.currentDisplayed = maxTraits;
            } else if (!isFirstLoad) {
                // Load more: mostrar siguiente batch
                const startIndex = this.loadMoreState.currentDisplayed;
                const endIndex = Math.min(startIndex + maxTraits, this.loadMoreState.allTraits.length);
                traitsToDisplay = this.loadMoreState.allTraits.slice(startIndex, endIndex);
                this.loadMoreState.currentDisplayed = endIndex;
                console.log(`📊 Cargando más traits: ${startIndex} a ${endIndex} (${traitsToDisplay.length} traits)`);
            } else {
                // Primera carga pero menos de 150 traits
                this.loadMoreState.currentDisplayed = tokens.length;
            }
        }
        ...
        // Añadir botón "Load More" si hay más traits para mostrar
        if (this.currentFilter === 'traits' && this.loadMoreState.currentDisplayed < this.loadMoreState.allTraits.length) {
            this.addLoadMoreButton(tokensGrid);
        }
    ...
    /**
     * Añadir botón "Load More" al final del grid
     */
    addLoadMoreButton(tokensGrid) {
        // Remove existing button if any
        const existingBtn = tokensGrid.querySelector('.load-more-button');
        if (existingBtn) {
            existingBtn.remove();
        }
        ...
        loadMoreBtn.textContent = `Load More (${this.loadMoreState.allTraits.length - this.loadMoreState.currentDisplayed} remaining)`;
        ...
        loadMoreBtn.addEventListener('mouseenter', () => {
            ...
            loadMoreBtn.style.borderColor = 'rgba(255, 255, 255, 0.5)';
        });
        
        loadMoreBtn.addEventListener('mouseleave', () => {
            loadMoreBtn.style.background = 'rgba(255, 255, 255, 0.1)';
            loadMoreBtn.style.borderColor = 'rgba(255, 255, 255, 0.3)';
        });
        
        loadMoreBtn.addEventListener('click', () => {
            console.log('🔄 Load More clicked, cargando más traits...');
            // Cargar más traits desde donde se quedó
            this.displayTokens(this.loadMoreState.allTraits, false, false);
        });
        
        tokensGrid.appendChild(loadMoreBtn);
    }
