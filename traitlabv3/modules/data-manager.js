...
class TraitLABDataManager {
    ...
    async init(force = false) {
        ...
        if (this.isInitialized && !force) {
            ...
            if (hasWallet && !hasTokens) {
                console.log('📊 DataManager: Wallet conectada pero sin tokens, reinicializando...');
                this.isInitialized = false;
                ...
                console.log('📊 DataManager: No hay wallet conectada, esperando conexión...');
                return;
            } else {
        ...
        if (!userAddress) {
            console.warn('📊 DataManager: No hay wallet conectada, no se pueden cargar tokens');
     ...
     * @deprecated Usar window.app.modules.ui.displayTokens() directamente
     */
    displayTokensImmediately(tokens, filter) {
        ...
        if (window.app?.modules?.ui) {
            ...
            try {
                // Usar nueva firma estandarizada
                window.app.modules.ui.displayTokens(tokens, { 
                    filter: filter,
                    hasLoadingWheels: true 
                });
