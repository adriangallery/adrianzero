...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  addPixels: (pixels) => {
    ...
    // Agregar todos los píxeles de una vez
    ...
    // Crear una sola entrada en el historial
    const newHistory = [...state.design.history.slice(0, state.design.historyIndex + 1), newPixels]
    if (newHistory.length > MAX_HISTORY) {
      newHistory.shift()
    }
  ...
  setBrushSoftness: (softness) => {
    // Validar rango 0-100
    const validSoftness = Math.max(0, Math.min(100, softness))
    set((state) => ({
      design: {
        ...state.design,
        brushSoftness: validSoftness,
      }
    }))
  },
  removePixel: (x, y) => {
