...
export function Canvas() {
  ...
  const {
    design: { pixels, currentColor, brushSize, tool, showGrid, pointerX, pointerY, showPointer, brushSoftness },
    addPixel, addPixels, setPointerPosition, setShowPointer,
    removePixel,
  } = useAppStore()
    ...
    if (x >= 0 && x < CANVAS_CONFIG.width && y >= 0 && y < CANVAS_CONFIG.height) {
      ...
      if (isPaintable) {
        if (tool === 'eraser') {
          ...
          if (brushSize === 1) {
            addPixel({ x, y, color: applyOpacity(currentColor, brushSoftness) })
          } else {
            const pixelsToAdd = []
            for (let dx = 0; dx < brushSize; dx++) {
              for (let dy = 0; dy < brushSize; dy++) {
                ...
                if (px < CANVAS_CONFIG.width && py < CANVAS_CONFIG.height) {
                  const isBrushPixelPaintable = tshirtMask[py] && tshirtMask[py][px]
                  if (isBrushPixelPaintable) {
                    pixelsToAdd.push({ x: px, y: py, color: applyOpacity(currentColor, brushSoftness) })
                  }
                }
              }
            }
  ...
  const handlePointerMove = (e: React.MouseEvent) => {
    ...
    // Check if coordinates are within canvas bounds
    if (x >= 0 && x < CANVAS_CONFIG.width && y >= 0 && y < CANVAS_CONFIG.height) {
      // Check if the pixel is in a paintable area
      const isPaintable = tshirtMask.length > 0 && tshirtMask[y] && tshirtMask[y][x]
      if (isPaintable) {
  ...
  // Función para aplicar opacidad basada en brush softness
  const applyOpacity = (color: string, softness: number): string => {
    // Si softness es 100, no aplicar opacidad (color sólido)
    if (softness >= 100) return color
    
    // Convertir softness a opacidad (0-1)
    const opacity = softness / 100
    
    // Si el color ya es RGBA, extraer los componentes
    if (color.startsWith('rgba')) {
      const rgba = color.match(/rgba?\(([^)]+)\)/)
      if (rgba) {
        const [r, g, b] = rgba[1].split(',').map(n => parseInt(n.trim()))
    ...
    // Para otros formatos, devolver el color original
    return color
  }
    ...
    <div className="retro-panel">
      ...
      <div className="flex justify-center">
        <div
          ...
          onMouseLeave={(e) => { handleMouseUp(); handlePointerLeave(); }}
        ...
        >
          ...
          {showPointer && (
            <div
              ...
              style={{
                left: pointerX * CANVAS_CONFIG.pixelSize * 1,
                top: pointerY * CANVAS_CONFIG.pixelSize * 1,
                width: brushSize * CANVAS_CONFIG.pixelSize * 1,
                height: brushSize * CANVAS_CONFIG.pixelSize * 1,
                backgroundColor: applyOpacity(currentColor, brushSoftness),
                border: `2px solid ${applyOpacity(currentColor, brushSoftness)}`,
                borderRadius: "2px",
                boxShadow: `0 0 4px ${applyOpacity(currentColor, brushSoftness)}80`,
              }}
            />
          )}
