...
// API key se carga desde config-keys.js (generado por GitHub Actions)
const ALCHEMY_API_KEY = (typeof window !== 'undefined' && window.ALCHEMY_KEYS_CONFIG?.primary) || null;
const ALCHEMY_RPC_URL = ALCHEMY_API_KEY ? `https://base-mainnet.g.alchemy.com/v2/${ALCHEMY_API_KEY}` : "https://mainnet.base.org";
...
// ===== UI THEME =====
// Background color for the whole app (change this value to whatever you want)
const APP_BACKGROUND_COLOR = '#07070b';
...
function applyBackgroundColor(color) {
    try {
        ...
        // Apply to screens if they don't already have their own background
        const screens = document.querySelectorAll('.screen');
        screens.forEach(screen => {
            // Only set inline background if none is set inline already
...
function initializeApp() {
    ...
    applyBackgroundColor(APP_BACKGROUND_COLOR);
    ...
    // Expose helper for quick testing in console: setBackground('#123456')
    window.setBackground = (c) => applyBackgroundColor(c);
}
