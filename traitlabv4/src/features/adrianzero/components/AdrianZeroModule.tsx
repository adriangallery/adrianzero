...
export function AdrianZeroModule() {
  ...
  // Mobile: sticky preview + categories, scrollable trait grid below
  // Desktop: fixed left panel + independently scrollable right panel
  if (isMobile) {
    return (
      <div className="flex flex-col h-full -mx-4 -mt-6">
        ...
        <div className="sticky top-0 z-20 bg-background border-b border-border">
          ...
          {/* Categories — horizontal scroll, single line */}
  ...
  // ─── Desktop: fixed left + scrollable right ──────────────

  return (
    <div className="flex flex-col h-full">
      {/* Top Bar */}
      <div className="flex items-center justify-between gap-2 py-2 border-b border-border mb-3 flex-shrink-0">
        <button onClick={handleClearNFTSelection} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
          <ChevronLeft className="h-4 w-4" />Back to collection
        </button>
        <div className="flex items-center gap-2">
          <button onClick={handlePrevNFT} disabled={selectedIndex <= 0} className="p-1.5 rounded-md hover:bg-muted disabled:opacity-30"><ChevronLeft className="h-4 w-4" /></button>
          <span className="text-xs font-medium text-foreground min-w-[80px] text-center">{displayName}</span>
          <button onClick={handleNextNFT} disabled={selectedIndex >= sortedTokens.length - 1} className="p-1.5 rounded-md hover:bg-muted disabled:opacity-30"><ChevronRight className="h-4 w-4" /></button>
        </div>
        {selectedTraits.length > 0 && (
          <div className="flex items-center gap-1.5">
            <span className="text-xs text-[#00ff00]">{selectedTraits.length} trait{selectedTraits.length !== 1 ? 's' : ''}</span>
            <button onClick={clearSelection} className="p-1 rounded hover:bg-muted"><X className="w-3 h-3 text-muted-foreground" /></button>
          </div>
        )}
      </div>
