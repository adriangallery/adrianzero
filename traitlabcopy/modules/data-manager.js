        ...
        try {
            ...
            if (window.app && window.app.modules.zero) {
                ...
                if (userAddress) {
                    ...
                    // 🚀 CARGAR TODOS LOS TRAITS SIN LÍMITE (para whales)
                    console.log('📊 Cargando TODOS los tokens ERC1155 (sin límite)...');
                    // Forzar modo completo (no batch) para cargar todos los traits
        ...
        try {
            ...
            try {
                if (batchMode) {
                    ...
                    // Modo completo: cargar TODOS los tokens sin límite
                    // Cargar en loop hasta que no haya más páginas
                    let allTokens = [];
                    let pageKey = null;
                    let hasMore = true;
                    let pageCount = 0;
