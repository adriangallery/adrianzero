...
class UIManager {
    constructor() {
        ...
        // Estado de paginación manual para traits (whales)
        this.traitsPagination = {
            currentPage: 1,
            itemsPerPage: 300,
            totalItems: 0,
            totalPages: 0,
            allTraits: [] // Almacenar todos los traits cargados
        };
     ...
     */
    setupEventListeners() {
        // Listener para actualizar visualización cuando cambia selección de traits
        if (window.app?.modules?.traits) {
            window.app.modules.traits.on('traitsSelectionUpdated', () => {
                if (this.currentFilter === 'traits') {
                    this.updateTraitsSelectionVisual();
                }
            });
            
            window.app.modules.traits.on('traitSelected', () => {
                if (this.currentFilter === 'traits') {
                    this.updateTraitsSelectionVisual();
                }
            });
            
            window.app.modules.traits.on('traitDeselected', () => {
                if (this.currentFilter === 'traits') {
                    this.updateTraitsSelectionVisual();
                }
            });
    ...
    displayTokens(tokens, skipSelectionUpdate = false, hasLoadingWheels = false) {
        ...
        // 🚨 PAGINACIÓN MANUAL: Para traits, usar paginación en lugar de límite
        ...
        // Deshabilitar lazy loading cuando hay paginación manual
        const shouldUseLazyLoading = this.isMobile() && 
                                     this.currentFilter === 'traits' && 
                                     this.traitsPagination.totalPages <= 1 && // Solo si hay 1 página o menos
        ...
        // Actualizar visualización de traits seleccionados (para paginación)
        if (this.currentFilter === 'traits') {
            this.updateTraitsSelectionVisual();
        }
    ...
    /**
     * Renderizar selector de página para traits
     */
    renderPageSelector() {
        const tokensGrid = this.domElements.get('tokens-grid');
        if (!tokensGrid || this.currentFilter !== 'traits') return;
        
        // Buscar o crear contenedor del selector
        let pageSelectorContainer = document.getElementById('traits-page-selector');
        if (!pageSelectorContainer) {
            // Crear contenedor antes del grid
            pageSelectorContainer = document.createElement('div');
            pageSelectorContainer.id = 'traits-page-selector';
            pageSelectorContainer.className = 'traits-page-selector';
            tokensGrid.parentNode.insertBefore(pageSelectorContainer, tokensGrid);
        }
        ...
        pageSelectorContainer.innerHTML = `
            ...
            <div class="page-selector-controls">
                <button class="page-btn" ${currentPage === 1 ? 'disabled' : ''} onclick="window.app.modules.ui.goToTraitsPage(${currentPage - 1})">
                    ← Anterior
                </button>
    ...
    goToTraitsPage(page) {
        ...
        // Re-renderizar traits de la nueva página
    ...
    /**
     * Actualizar visualización de traits seleccionados (mantener selección entre páginas)
     */
    updateTraitsSelectionVisual() {
        const tokensGrid = this.domElements.get('tokens-grid');
        if (!tokensGrid || this.currentFilter !== 'traits') return;
        
        // Obtener traits seleccionados desde traits module
        const selectedTraits = window.app?.modules?.traits?.getSelectedTraits() || [];
        const selectedTraitIds = new Set(selectedTraits.map(t => t.tokenId));
        
        // Marcar visualmente los traits seleccionados
        tokensGrid.querySelectorAll('.token-card').forEach(card => {
            const tokenId = parseInt(card.getAttribute('data-token-id'));
            if (selectedTraitIds.has(tokenId)) {
        ...
        if (token.tokenType === 'ERC721') {
            ...
            if (this.currentFilter === 'floppy') {
                ...
                if (token.tokenType === 'ERC721') {
                    ...
                    // Actualizar visualización de selección para paginación
                    if (this.currentFilter === 'traits') {
                        // La selección se maneja en traits module, solo actualizar visual
                        setTimeout(() => {
                            this.updateTraitsSelectionVisual();
                        }, 100);
                    }
